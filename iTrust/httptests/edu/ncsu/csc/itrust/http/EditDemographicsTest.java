...
public class EditDemographicsTest extends iTrustHTTPTest {
	...
	/**
	 * Tests to see if the patient demographic webpage is updating the form
	 * correctly after editing a piece of their demographic information.
	 * 
	 * If the bug still exits, this test will fail.
	 * Otherwise, the bug has been fixed.
	 * @author Kevin Commons (kjcommon)
	 * @throws Exception
	 */
	public void testEditDemographics8() throws Exception {
		// login as Patient 2
		WebConversation wc = login("2", "pw");
		WebResponse wr = wc.getCurrentPage();
		assertEquals("iTrust - Patient Home", wr.getTitle());
		...
		patientForm.setParameter("streetAddress1", "100 New Address");
		patientForm.getSubmitButtons()[0].click();
		wr = wc.getCurrentPage();
		assertTrue(wr.getText().contains("Information Successfully Updated"));
		assertEquals("100 New Address", wr.getForms()[0].getParameterValue("streetAddress1"));
	}
}
